...
export function readKey(text) {
    var key_index = determine_key(text);
    var key;
...
export function transpose(raw_text, num_steps) {   
    var key_index = determine_key(raw_text); 
        ...
        if (chord_line(text[i])) {
            ...
            for (let j=0; j<text[i].length; j++) {
                ...
                if ((/[a-zA-Z1-9#]/).test(text[i][j])) {
                    ...
                    if (chord.length != 0) {
                        var new_chord = transpose_chord(chord, num_steps, key_index); 
...
export function chord_line(line) {
...
export function remove_chord_lines(raw_text) {
    var text = raw_text.split("\n");
    var newtext = ""
...
export function chord_to_val(chord) {
...
export function val_transpose(val, num_steps) {
    var new_val;
...
export function chord2number(input_chord, origkey) {
...
export function transpose_chord(input_chord, num_steps, key_index) {
...
export function counter(input_array) {
    var return_array = [];
...
export function infer_key(chords) {
    var val_list = [];
    var minor;
    var prob;
...
export function increment_key(key, direction) {
...
export function key_difference(key1, key2) {
